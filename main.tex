...
%   - config/document.tex      - Document type, SSIC, Date, From, To, Via, Subject
%   - config/letterhead.tex    - Unit name and address
%   - config/classification.tex - CUI markings and distribution
%   - config/signatory.tex     - Signature block (full and abbreviated)
%   - config/references.tex    - Reference list
%   - config/reference-urls.tex - Clickable URLs for references
%   - config/enclosures.tex    - Enclosure list (PDFs go in enclosures/ folder)
%   - config/body.tex          - Document body content
%
% Format-specific code is in the formats/ folder
%
%=============================================================================
...
\newcommand{\setSignatory}[5]{%
    ...
    % Build full name for MFR/Business (First M. LASTNAME)
    \renewcommand{\SignatoryName}{#1 #2 \MakeUppercase{#3}}%
    % Build abbreviated for Naval Letter (F. M. LASTNAME)
    % Extract first character of first name
    \def\@tempfirst{#1}%
    \expandafter\def\expandafter\@firstchar\expandafter{\expandafter\@car\@tempfirst\@nil}%
    \edef\SignatoryAbbrev{\@firstchar. #2 \MakeUppercase{#3}}%
}
\makeatother
        ...
        \loop\ifnum\value{inclcount}<\value{enclmax}%
            ...
            \@ifundefined{encl@defined@\arabic{inclcount}}{}{%
                ...
                \ifx\currentfile\empty
                    % No PDF - create placeholder page
                    \newpage
...
\input{config/letterhead}
\input{config/classification}
\input{config/document}
\input{config/signatory}
