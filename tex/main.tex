...
% Flag set by \setHasReferences command from JavaScript (in preamble)
\newif\ifhasrefs
\hasrefsfalse
...
% In preamble, refitem just sets the flag (no output)
% We redefine it after \begin{document} to produce output
\newcommand{\refitem}[2]{\setHasReferences}

% Pre-load references in preamble to set the hasrefs flag
\input{references}

% SECNAV M-5216.5 Ch 7, Para 10.c: 4 spaces after "Ref:"
% Only print if hasrefs flag is set
...
% Redefine refitem after document begins to actually output references
\AtBeginDocument{%
    \makeatletter
    \renewcommand{\refitem}[2]{%
