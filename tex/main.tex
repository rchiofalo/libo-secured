...
% Counter to track number of references
\newcounter{refcount}
...
% Pre-load references to count them (sets refcount)
\input{references}

% SECNAV M-5216.5 Ch 7, Para 10.c: 4 spaces after "Ref:"
% Only print if there are references
\newcommand{\printReferences}{%
    \ifnum\value{refcount}>0
        \vspace{12pt}
        \noindent
        \begin{tabular}{@{}l@{}p{5.9in}@{}}
            Ref:\hspace{4\fontdimen2\font} & \begin{minipage}[t]{5.9in}
                       % Reset counter and re-input to actually display
                       \setcounter{refcount}{0}%
                       \input{references}
                   \end{minipage}
        \end{tabular}%
