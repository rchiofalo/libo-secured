...
% Flag to track if any references exist (set by JavaScript in references.tex)
...
\makeatletter
\newcommand{\refitem}[2]{%
    \hasreferencestrue% Mark that we have at least one reference
...
% References are loaded inside a box to check if any exist, then displayed
\newsavebox{\refbox}
\newcommand{\printReferences}{%
    % First, silently load references into a box to set the hasreferences flag
    \savebox{\refbox}{\parbox{5.9in}{\input{references}}}%
    % Only print if we have references
    \ifhasreferences
        \vspace{12pt}
        \noindent
        \begin{tabular}{@{}l@{}p{5.9in}@{}}
            Ref:\hspace{4\fontdimen2\font} & \usebox{\refbox}
        \end{tabular}%
    \fi
}
