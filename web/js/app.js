...
async function compileLatex() {
    ...
    // Write enclosure PDF files to virtual filesystem for \includepdf
    for (let i = 0; i < enclosures.length; i++) {
        const encl = enclosures[i];
        if (encl.file && encl.file.data) {
            // Use a sanitized filename: encl_1.pdf, encl_2.pdf, etc.
            const filename = `encl_${i + 1}.pdf`;
            const bytes = new Uint8Array(encl.file.data);
            pdfTexEngine.writeMemFSFile(`enclosures/${filename}`, bytes);
            console.log(`Wrote enclosure: ${filename} (${bytes.length} bytes)`);
        }
    }
 ...
 * Enclosure PDFs are written to the virtual filesystem as encl_1.pdf, encl_2.pdf, etc.
 * LaTeX uses \includepdf to embed them with proper formatting (scale, border, headers/footers).
 */
...
${enclosures.map((e, i) => {
    // If enclosure has a file attached, use sanitized filename (encl_N.pdf)
    // If no file, pass empty (LaTeX creates placeholder page)
    const filename = (e.file && e.file.data) ? `encl_${i + 1}.pdf` : '';
 ...
 * Note: Enclosure PDFs are embedded by LaTeX via \includepdf with proper formatting.
 */
async function downloadLatexPDF() {
    try {
        showStatus('Compiling LaTeX...', 'success');

        // Compile LaTeX document (enclosures are embedded via \includepdf)
