 ...
 * Note: For enclosures WITH attached PDF files, we pass empty filename so LaTeX
 * skips them - the actual PDFs are merged by JavaScript (pdf-lib) after compilation.
 * For enclosures WITHOUT files, LaTeX creates placeholder pages.
 */
function generateEnclosuresTex() {
    if (enclosures.length === 0) return '% No enclosures\n';
    return `%=============================================================================
% ENCLOSURES - Generated by libo-secured
% Note: PDF files are merged after compilation, not via \\includepdf
%=============================================================================

${enclosures.map((e, i) => {
    // If enclosure has a file attached, pass empty filename (JS will merge the PDF)
    // If no file, also pass empty (LaTeX creates placeholder page)
